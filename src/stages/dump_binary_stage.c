...
/* raw_record/validated/extracted/wire_frame -> nothing (a leaf, see
 * its plugin shim's out_port_count). Writes every record's bytes
 * verbatim to a file, in order, no framing/length-prefixing added -
 * the generic "capture whatever's flowing at this point in the graph"
 * debugging tool, since all four of its accepted types are already
 * opaque byte blobs with no semantic value needing interpretation
 * (unlike PORT_TYPE_ENGINEERING - see forward_udp_stage.c's header
 * comment for why that distinction matters for a stage like this). */
